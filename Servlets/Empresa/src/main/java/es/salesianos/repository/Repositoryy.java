...
public class Repositoryy {
	...
	public List<Student> searchAllStudent() {
		List<Student> listCompany = new ArrayList<Student>();
		Connection conn = manager.open(jdbcUrl);
		ResultSet resultSet = null;
		PreparedStatement prepareStatement = null;
		try {
			...
			resultSet = prepareStatement.executeQuery();
			while (resultSet.next()) {
				Student CompanyInDatabase = new Student();
				
				CompanyInDatabase.setId(resultSet.getInt(1));
				CompanyInDatabase.setName(resultSet.getString(2));
				CompanyInDatabase.setAge(resultSet.getInt(3));
				CompanyInDatabase.setAsisteFCT(resultSet.getBoolean(4));
