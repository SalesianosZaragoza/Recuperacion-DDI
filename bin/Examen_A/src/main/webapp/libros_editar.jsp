<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<%@ page import="es.salesianos.model.Libro,java.util.List,es.salesianos.model.Almacen" %>
		...
		<div class="p-4">
			...
			<form action="editarLibro" method="post">
				<div class="form-group">
					...
					<input id="id" type="text" name="id" value="<%=libro.getId()%>" readonly>
				</div>
				<div class="form-group">
				...
				<div class="form-group">
					...
					<input id="fechaEdicion" type="date" name="fechaEdicion" value="<%=libro.getShortDate()%>" onchange="onCambiarFecha(this)">
				</div>
				<div class="form-group">
					<label>Almacen:</label>
					<% List<Almacen> almacenes_old = (List<Almacen>)request.getAttribute("almacenes_old"); %>
					<select id="almacenes_old" name="almacenes_old" autocomplete="off" ${libro.getAlmacen_new() == -1 ? "style='display: inline-block'" : "style='display: none'"}>
						<c:forEach var="almacen" items="${almacenes_old}">
							<option value="${almacen.getId()}" ${almacen.getId().equals(libro.getAlmacen().getId()) ? "selected='selected'" : ""}>
								<c:out value="${almacen.getName()}"/>
					    	</option>
						</c:forEach>
					</select>
					<% List<Almacen> almacenes_new = (List<Almacen>)request.getAttribute("almacenes_new"); %>
					<select id="almacenes_new" name="almacenes_new" autocomplete="off" ${libro.getAlmacen_old() == -1 ? "style='display: inline-block'" : "style='display: none'"}>
